	...
	module V1
		...
		respond_to :json
			...
			def index
				...
				@friends_list = []
				@friends.select {|f| @friends_list << {:name => User.find_by_reecher_id(f.friend_reecher_id.strip).full_name, :email => User.find_by_reecher_id(f.friend_reecher_id).email} } if @friends.size > 0
