...
describe 'Solver use case (a1: p1>c1, a2: p2>c1, p3>c2, a3: p4>c3)' do
  before do
    @c1 = Category.create!
    @c2 = Category.create!
    @c3 = Category.create!
  ...
  describe "u1 solutions on p2" do
    before do
      @u1 = User.create!
      @solution = @p2.solutions.create! :user => @u1
    end
    
    it "u1.solutions should == [solution]" do
      @u1.solutions.should == [@solution]
    end
    
    it "a1.solvers should be empty" do
      @a1.solvers.should be_empty
    end
    
    it "a2.solvers should == [u1]" do
      @a2.solvers.should == [@u1]
    end
    
    it "u1.solved_posts should == [p2]" do
      @u1.solved_posts.should == [@p2]
    end
    
    it "u1.solved_posts.find_inflamatory(:all) should be empty" do
      @u1.solved_posts.find_inflamatory(:all).should be_empty
    end
    
    if ActiveRecord::Base.respond_to?(:named_scope)
      it "u1.solved_posts.inflamatory should be empty" do
        @u1.solved_posts.inflamatory.should be_empty
      end
    end
    
    it "u1.solved_authors should == [a2]" do
    ...
    describe "when p2 is inflamatory" do
      ...
      it "u1.solved_posts.find_inflamatory(:all) should == [p2]" do
        # uniq ids is here (and next spec) because eager loading changed behaviour 2.0.2 => edge
        @u1.solved_posts.find_inflamatory(:all).collect(&:id).uniq.should == [@p2.id]
      end
      ...
      if ActiveRecord::Base.respond_to?(:named_scope)
        it "u1.solved_posts.inflamatory should == [p2]" do
